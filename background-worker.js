// Main background script worker
// No ES module imports here - just standard service worker script
  ...
  if (port.name === 'yt-transcript-content') {
    ...
    port.onMessage.addListener((msg) => {
      if (msg.action === 'CONTENT_SCRIPT_READY') {
        // Use the correct tabId from the sender
        const tabId = port.sender.tab.id;
        console.log(`Content script ready in tab ${tabId}`);
        
        // Check if we need to send the video ID
        if (port.sender.tab && port.sender.tab.url.includes('youtube.com/watch')) {
          const videoId = new URL(port.sender.tab.url).searchParams.get('v');
          if (videoId) {
            port.postMessage({
              action: 'NEW_VIDEO_LOADED',
              videoId: videoId
...
// Enhanced function to validate API key using REST API
